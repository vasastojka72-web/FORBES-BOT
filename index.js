...
function canDisciplineAll(member,req){return _mainId(req)||_hasAnyRole(member,["лідер","leader","зам","права рука"]);}
...
async function getPublicMembersFromDiscord(){
  try{
    ...
    const familyRoleNames = ["лідер","leader","зам","заступ","права рука","старший каптер","каптер","старший фармер","фармер","фарм","учасник","forbes"];
    return Array.from(collection.values())
      .filter(m=>!m.user?.bot)
      .map(m=>{
        const display = m.displayName || m.nickname || m.user?.username || "Unknown";
        const parsed = parseForbesStaticNick(display);
        const roles = cleanPublicRolesForbes(m);
        const familyRoles = roles.filter(r=>familyRoleNames.some(w=>String(r.name).toLowerCase().includes(w)));
        const visibleRoles = familyRoles.length ? familyRoles : roles;
        const mainRole = visibleRoles[0]?.name || "Учасник";
        return {
          nick: parsed.nick,
          ...
          role: mainRole,
          rank: mainRole,
          roles: visibleRoles.map(r=>({name:r.name}))
        };
      })
...
app.get("/api/public-dashboard", async (req,res)=>{
  try{
    ...
    res.set("Cache-Control","no-store, no-cache, must-revalidate, private");
    res.json({ok:true,membersCount:members.length,carsCount:cars.length,galleryCount:gallery.length,members,cars,gallery,photos:gallery,familyInfo});
  }catch(e){
